...
public class Stu_process {
    ...
    void stu_delete(ArrayList list) {
        ...
        // 삭제코드 추가하면 됨.
//		String name = ((Students) list.get(temp_no)).name; -> 이러면 s.name을 name 이라 써도됨.
		Students s = (Students) list.get(temp_no);
	...
	void stu_search(ArrayList list) {
		System.out.printf("[ 학생검색 ]");
		System.out.println("---------------------------------");
		System.out.println("1. 학생이름 수정");
		System.out.println("2. 합계점수 수정");
		System.out.println("3. 평균점수 수정");
		System.out.println("-----------------------");
		System.out.println("원하는 번호를 입력하세요.>> ");
		choice = scan.nextInt();
		scan.nextLine();
		...
		switch (choice) {
			case 1:
				...
				break;
			case 2: // 합계점수 검색
				System.out.println("몇점이상의 합계점수를 검색하시겠습니까?>>");
				...
				break;
			case 3:
				System.out.println("몇점이상의 평균점수를 검색하시겠습니까?>>");
